    ...
    setDebts((prev) =>
      prev.map((d) => {
        ...
        const updatedPayments = isDecrease
          ...
          : d.payments;
        // If the new balance exceeds the tracked original (e.g. new charges/fees),
        // raise originalBalance so the "of X" reference and progress % stay coherent.
        const originalBalance = Math.max(d.originalBalance, newBalance);
        return { ...d, balance: newBalance, originalBalance, updatedAt: now, payments: updatedPayments };
      })
    );
